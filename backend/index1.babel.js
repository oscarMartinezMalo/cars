...
api.get('/cars/:page/:perPage/:sortByprice/:search?', function (req, res) {
    ...
    db.query(sql, function (err, resTotal) {
        if (err) {
            ...
            var sql1 = 'SELECT `stock-number` as stockNumber, `car-name` as carName, `price-difference` as priceDifference, `engine`, `tranmission`, `mpg-range` as mpgRange,`exterior-color` as exteriorColor FROM `cars`.`doral-hundai` ' + searchQuery + ' order by `internet-price`' + sortByprice + ' limit ' + perPage + ' offset ' + offsetNumber;
            console.log(sql1);
